...
def evaluate_model(tokenizer,df, df_exp, uids, uid2idx, uid2text, ranks, preds, save_model, model_with_no_token_types, model_name='roberta',model_type='roberta-base', mode='train', top_k = 100 ):
    if mode=='train':
        ...
        if not os.path.exists('results.csv'):
            ...
            result_df = po.read_csv('results_train.csv')
        ...
        result_df.to_csv('results_train.csv',index=False)
        ...
        if not os.path.exists('results.csv'):
            ...
            result_df = po.read_csv('results_dev.csv')
        results={'Model':save_model,'TFIDF_MAP_dev':np.mean(np.array(direct_aps)),'Reranked_MAP_dev':np.mean(np.array(reranked_aps))}
        result_df = result_df.append(results, ignore_index=True)
        result_df.to_csv('results_dev.csv',index=False)
